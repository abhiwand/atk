...
*/
  ...
  val assignSampleDoc = CommandDoc(oneLineSummary = "Assign classes to rows.",
    extendedSummary = Some("""
                            ...
                            |   generated by the probability distribution function specified by a list
  ...
  */
  ...
  val columnModeDoc = CommandDoc(oneLineSummary = "Calculate modes of a column.",
    extendedSummary = Some("""
                            ...
                            |
                            |   Because data distributions often have mutliple modes, it is possible for
                            |   a set of modes to be returned.
                            |   By default, only one is returned, but my setting the optional parameter
                            |   max_number_of_modes_returned, a larger number of modes can be returned.
                            |
                            |   Parameters
                            |   ----------
                            |   data_column : str
                            ...
                            |               If the number of modes is <= the parameter
                            |               maxNumberOfModesReturned, then all modes of the data are
                            |               returned.
                            |               If the number of modes is > maxNumberOfModesReturned, then
                            |               only the first maxNumberOfModesReturned many modes
                            |               (per a canonical ordering) are returned.
                            |       *   weight_of_mode : Weight of a mode.
                            |               If there are no data elements of finite weight > 0,
                            |               the weight of the mode is 0.
                            |               If no weights column is given, this is the number of
                            |               appearances of each mode.
                            |       *   total_weight : Sum of all weights in the weight column.
                            |               This is the row count if no weights are given.
                            |               If no weights column is given, this is the number of rows
...
TODO: delete me, code moved to separate plugin files
  ...
  val columnMedianDoc = CommandDoc(oneLineSummary = "Calculate (weighted) median of a column.",
    extendedSummary = Some("""
                            ...
                            |   The median is the least value X in the range of the distribution so that the
                            |   cumulative weight of values strictly below X is strictly less than half of
                            |   the total weight and the cumulative weight of values up to and
                            |   including X is >= 1/2 the total weight.
                            |
                            ...
                            |       calculation.
                            |       Must contain numerical data.
                            |       Uniform weights of 1 for all items will be used for
                            ...
                            |       so a column of Longs will result in a Long median and a column of Floats
                            |       will result in a Float median.
                            |
                            |   Example
                            |   -------
                            |   ::
...
*/
  ...
  val columnSummaryStatisticsDoc = CommandDoc(oneLineSummary = "Calculate summary statistics of a column.",
    extendedSummary = Some("""
                            ...
                            |               of each data element from the weighted mean, divided by the
                            |               total weight.
                            |       *   standard_deviation:
                            |               The square root of the variance.
                            |               None when sample variance is being used and the sum of weights
                            |               is <= 1.
                            |       *   valid_data_count:
                            ...
                            |               None when there are no elements of positive weight.
                            |       *   bad_row_count:
                            |               The number of rows containing a NaN or infinite value in either
                            |               the data or weights column.
                            |       *   good_row_count:
                            |               The number of rows not containing a NaN or infinite value in
                            |               either the data or weights column.
                            |       *   positive_weight_count:
                            |               The number of valid data elements with weight > 0.
                            |               This is the number of entries used in the statistical
                            |               calculation.
                            ...
                            |       .. math::
                            |
                            |           \\left( \\frac{1}{W - 1} \\right) * sum_{i}  \\left(x_{i} - M \\
                            |           \\right) ^{2}
                            |
                            ...
                            |       .. math::
                            |
                            |           \\left( \\frac{1}{W} \\right) * sum_{i}  \\left(x_{i} - M \\
                            |           \\right) ^{2}
                            |
                            |       where :math:`W` is sum of weights over valid elements of positive weight,
                            |       and :math:`M` is the weighted mean.
                            ...
                            |   --------------------
                            |
                            |   A row is bad when it contains a NaN or infinite value in either its data or
                            |   weights column.
                            |   In this case, it contributes to bad_row_count; otherwise it contributes to
                            |   good row count.
                            |
                            ...
                            |   otherwise (when the weight is > 0) it contributes to
  ...
  */
  ...
  val dropColumnsDoc = CommandDoc(oneLineSummary = "Remove columns from the frame.",
    extendedSummary = Some("""
                            ...
                            |   Examples
                            |   --------
                            |   For this example, BigFrame object *my_frame* accesses a frame with columns
                            |   *column_a*, *column_b*, *column_c* and *column_d*.
  ...
  */
  ...
  val quantileDoc = CommandDoc(oneLineSummary = "Calculate quantiles on given column.",
    extendedSummary = Some("""
                           ...
                           |    The key is the quantile and the value is the quantile value.
                           |    ::
                           |
  ...
  val classificationMetricsDoc = CommandDoc(oneLineSummary = "Model descriptors",
    extendedSummary = Some("""
                           |    Based on the *metric_type* argument provided, computes the accuracy,
                           |    precision, recall or :math:`F_{\\beta}` measure for a classification model
                           |
                           |    *   When metric_type provided is 'f_measure': Computes the
                           ...
                           |        \\frac{\\frac{T_{P}} {T_{P} + F_{P}} \\
                           |        * \\frac{T_{P}}{T_{P} + F_{N}}}{\\beta ^ 2 * \\
                           |        \\left(\\frac{T_{P}}{T_{P} + F_{P}} + \\frac{T_{P}}{T_{P} + \\
                           |        F_{N}}\\right)}
                           | 
                           ...
                           |        each label, where the weight is the number of instance with each label
                           |        in the labeled column.
                           |        The determination of binary vs. multi-class is automatically inferred
                           |        from the data.
                           | 
                           |    *   When metric_type provided is 'recall': Computes the recall measure for
                           ...
                           |        from the data.
                           | 
                           |    *   When metric_type provided is 'precision': Computes the precision measure
                           ...
                           |        from the data.
                           | 
                           |    *   When metric_type provided is 'accuracy': Computes the accuracy measure
                           |        for a classification model.
                           |        A column containing the correct labels for each instance and a column
                           ...
                           |        :math:`T_{N}` denote the number of true negatives, and :math:`K`
                           |        denote the total number of classified instances, then the model accuracy
                           |        is given by: :math:`\\frac{T_{P} + T_{N}}{K}`.
                           | 
                           ...
                           |    pos_label : str
                           |        the value to be interpreted as a positive instance (only for binary,
                           |        ignored for multi-class)
                           |    beta : float
                           |        beta value to use for :math:`F_{\\beta}` measure (default F1 measure is
                           |        computed); must be greater than zero
                           | 
                           |    Returns
                           |    -------
                           |    float64
                           ...
                           |          green             0              1                  1
                           | 
                           |        frame.classification_metrics('f_measure', 'labels', 'predictions', '1',
                           |        1)
                           | 
                           |        0.66666666666666663
                           | 
                           ...
                           |        0.55555555555555558
                           | 
                           |        frame.classification_metrics('f_measure', 'labels', 'predictions', '0',
                           |        1)
                           | 
                           |        0.80000000000000004
                           | 
                           ...
                           |        1.0
                           | 
                           |        frame.classification_metrics('precision', 'labels', 'predictions', '1',
                           |        1)
                           | 
                           |        1.0
                           | 
                           |        frame.classification_metrics('precision', 'labels', 'predictions', '0',
                           |        1)
                           | 
                           |        0.66666666666666663
                           | 
                           ...
                           |        0.75
                           | 
                           |        frame.classification_metrics('confusion_matrix', 'labels',
                           |        'predictions', '1', 1)
                           | 
                           |        [1, 2, 0, 1]
                           | 
  ...
  val tallyPercentDoc = CommandDoc(oneLineSummary = "Computes a cumulative percent count.",
    extendedSummary = Some("""
                           |    A cumulative percent count is computed by sequentially stepping through the
                           |    column values and keeping track of the current percentage of the total
                           |    number of times the specified *count_value* has been seen up to the current
                           |    value.
                           | 
                           |    Parameters
                           |    ----------
                           ...
                           |        appended with a column containing the cumulative percent counts
                           | 
                           |    Examples
                           |    --------
                           |    Consider BigFrame *my_frame*, which accesses a frame that contains a single
                           ...
                           | 
                           |    The BigFrame *cpc_frame* accesses a new frame that contains two columns,
                           |    *obs* that contains the original column values, and
  ...
  val tallyDoc = CommandDoc(oneLineSummary = "Computes a cumulative count.",
    extendedSummary = Some("""
                           ...
                           |    values and keeping track of the number of times the specified *count_value*
                           |    has been seen, up to the current value.
                           | 
                           |    Parameters
                           |    ----------
                           |    sample_col : string
  ...
  val cumSumDoc = CommandDoc(oneLineSummary = "Computes a cumulative sum.",
    extendedSummary = Some("""
                           ...
                           |    -------
                           |    BigFrame
                           |        A new object accessing a frame containing the original columns appended
                           |        with a column containing the cumulative sums
                           | 
                           |    Notes
                           |    -----
                           |    This function applies only to columns containing numerical data.
                           ...
                           | 
                           |    The BigFrame *cs_frame* accesses a new frame that contains two columns,
                           |    *obs* that contains the original column values, and *obsCumulativeSum*
                           |    that contains the cumulative percent count::
                           | 
                           |        cs_frame.inspect()
                           | 
