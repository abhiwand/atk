...
import com.intel.intelanalytics.service.v1.decorators.{ CommandDecorator, FrameDecorator }
                ...
                (offset, count) =>
                  {
                    ...
                    val commandUri = pattern.replaceFirstIn(uri.toString, "commands/") + exec.start.id
                    complete(CommandDecorator.decorateEntity(commandUri, List(Rel.self(commandUri)), exec.start))
                  }
              }
            }
