...
public class HBaseReaderMapperTest {
	...
	public final void setUp() throws Exception {
		...
		spiedBaseMapper.setMapVal(SerializedGraphElementStringTypeVids.class
				.newInstance());
		doNothing().when(spiedBaseMapper).setMapVal(
	...
	public final void verify_writes_with_good_input() throws Exception {
		...
		assertEquals(
				...
				mapDriver
						...
						.getValue());
		assertEquals("verify context row counter", 1, mapDriver.getCounters()
				.findCounter(GBHTableConfiguration.Counters.HTABLE_ROWS_READ)
				.getValue());

		// make sure the error counters are 0
		assertEquals(
				"verify context edge error counter",
				0,
				mapDriver.getCounters()
						.findCounter(BaseMapper.getEdgeWriteErrorCounter())
						.getValue());
		assertEquals(
				"verify context vertex error counter",
				0,
				mapDriver.getCounters()
	 ...
	 * @return
	 */
	public final KeyValue newKeyValue(String row, String cf, String qualifier,
	 ...
	 *            the type of graph element edge/vertex
	 * @param edge
	 *            the edge object to verify against
	 * @param vertex
	 *            the vertex object to verify against
	 */
	public final void verifyPairSecond(
			Pair<IntWritable, SerializedGraphElement> pair,
			String graphElementType, Edge<StringType> edge,
