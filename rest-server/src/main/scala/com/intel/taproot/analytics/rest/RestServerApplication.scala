...
import com.intel.taproot.analytics.rest.factory.ServiceFactoryCreator
...
class RestServerApplication(archiveDefinition: ArchiveDefinition, classLoader: ClassLoader, config: Config)
  ...
  EventLogging.raw = configuration.getBoolean("intel.taproot.analytics.api.logging.raw")
  info("REST server set log adapter from configuration")

  EventLogging.profiling = configuration.getBoolean("intel.taproot.analytics.api.logging.profile")
