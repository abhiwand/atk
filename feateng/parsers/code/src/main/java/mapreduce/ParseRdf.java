...
import mapreduce.Rdf;
import java.io.IOException;

public class ParseRdf {

  private static final Log LOG = LogFactory.getLog(ParseRdf.class);

  public static final String NAME = "ParseRdf";
    ...
    throws IOException {
      ...
      try {
        ...
        for (KeyValue kv : columns.list()) {
          ...
          Rdf rdf = new Rdf();
          rdf.setNTriples(value.toString());
          
          put.add(columnFamily, Bytes.toBytes("subject"),Bytes.toBytes(rdf.getSubject().toString()) );
          put.add(columnFamily, Bytes.toBytes("predicate"),Bytes.toBytes(rdf.getPredicate().toString()) );
          put.add(columnFamily, Bytes.toBytes("object"),Bytes.toBytes(rdf.getObject().toString()) );
        ...
        System.err.println("Error: " + e.getMessage() + ", Row: " +
          Bytes.toStringBinary(row.get()) + ", RDF: " + value);
  ...
  public static void main(String[] args) throws Exception {
    ...
    Job job = new Job(conf, "Parse data in " + input + ", write to " + output +
      "(map only)");
    job.setJarByClass(ParseRdf.class);
