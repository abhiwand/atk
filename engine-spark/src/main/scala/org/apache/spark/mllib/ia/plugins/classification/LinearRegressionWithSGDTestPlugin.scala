...
/* Run the LinearRegressionWithSGD model on the test frame*/
class LinearRegressionWithSGDTestPlugin extends SparkCommandPlugin[ClassificationWithSGDTestArgs, ClassificationMetricValue] {
  /**
  ...
  override def execute(arguments: ClassificationWithSGDTestArgs)(implicit invocation: Invocation): ClassificationMetricValue =
    {
      ...
      val linRegJsObject = modelMeta.data.get
      val linRegData = linRegJsObject.convertTo[LinearRegressionData]
      val linRegModel = linRegData.linRegModel
      if (arguments.observationColumns.isDefined) {
        require(linRegData.observationColumns.length == arguments.observationColumns.get.length, "Number of columns for train and test should be same")
      }
      val linRegColumns = arguments.observationColumns.getOrElse(linRegData.observationColumns)

      val labeledTestRdd: RDD[LabeledPoint] = testFrameRdd.toLabeledPointRDD(arguments.labelColumn, linRegColumns)
