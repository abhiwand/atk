...
public class CommandLineInterfaceTest {
	...
	private static String[] haddopGenericOptionsExample1 = { "-conf",
			configFile, "-Dtest=test" };
	private static String[] haddopGenericOptionsExample2 = { "-conf",
			configFile, "-D", "test=test" };
	...
	public void setUp() throws Exception {
		...
		PowerMockito
				.doNothing()
				.when(GraphBuilderExit.class,
						method(GraphBuilderExit.class,
								"graphbuilderExitNoException", StatusCode.class))
	...
	public void test_hadoop_generic_options_are_parsed() {
		...
		assertEquals("verify parsed conf option", configFile,
				spiedCLI.getGenericOptionsParser().getCommandLine()
						.getOptionValue("conf"));
		assertEquals("verify parsed single option", "test=test", spiedCLI
				.getGenericOptionsParser().getCommandLine().getOptionValue("D"));
	...
	public void test_custom_option_parsing() {
		...
		String[] customArgs = { "-one", "we parsed -one", "-two",
				"we parsed -two" };
		String[] cmdArgs = (String[]) ArrayUtils.addAll(
				haddopGenericOptionsExample1, customArgs);
	...
	public void test_custom_option_parsing_revers_order() {
		...
		String[] customArgs = { "-two", "we parsed -two", "-one",
				"we parsed -one" };
		String[] cmdArgs = (String[]) ArrayUtils.addAll(
				haddopGenericOptionsExample1, customArgs);
		spiedCLI.checkCli(cmdArgs);
	...
	/**
	 * given two hashmaps return a random order string array with all the key
	 * value pairs from both maps
	 * 
	 * @param hadoopArgs
	 *            the list of hadoop options to randomize
	 * @param gbArgs
	 *            the list of graph builder specific options to randomize
	 * @return a string array that mimics the main input args array
	 */
	public String[] getRandomizedCommandLine(
			HashMap<String, String> hadoopArgs, HashMap<String, String> gbArgs) {
	 ...
	 * 
	 * @param hash
	 *            the hash map to derive our random key-value pair from
	...
	/**
	 * verify that the option is parsed and matches our expected value after
	 * it's parsed by the command line interface
	 * 
	 * @param option
	 *            the option to verify against
	 * @param expected
	 *            the expected parsed value
	 * @param cli
	 *            instance of the CommandLineInterface class to assert against
	 */
	private void testParsedOptions(Option option, String expected,
			CommandLineInterface cli) {
		if (spiedCLI.hasOption(option.getOpt())
				&& spiedCLI.hasOption(option.getLongOpt())) {
			assertTrue(String.format(
					"check the parsed value against the short option: %s",
					option.getOpt()), cli.getOptionValue(option.getOpt())
					.equals(expected));
			assertTrue(String.format(
					"check the parsed value against the long option: %s",
					option.getOpt()), cli.getOptionValue(option.getLongOpt())
					.equals(expected));
		} else {
			fail();
		}
	...
	/**
	 * test the amount of remaining arguments we have after the hadoop parser is
	 * done.
	 * 
	 * @param remaining
	 *            what is the expected amount of remaining arguments
	 * @param cli
	 *            the CommandLineInterface instance to grab the
	 *            hadoopGenericParser
	 */
	private void testRemainingHadoopArgs(int remaining, CommandLineInterface cli) {
		assertEquals(String.format(
				"look for the remaining args in this case we should have: %d ",
				remaining), remaining, cli.getGenericOptionsParser()
				.getRemainingArgs().length);
	}

	/**
	 * bootstrapping for testing a demo app. it will pull all the private static
	 * option configs and used them to verify that it got parsed and that the
	 * correct number of args remain after the hadoop generic options config
	 * 
	 * @param klass
	 *            the class of the demo app we will test
	 * @param demoAppCliArgs
	 *            hash map with the user command line args <option name, command
	 *            line value>
	 */
	private void testDemoApp(Class klass, HashMap<String, String> demoAppCliArgs)
			throws Exception {
		CommandLineInterface demoAppCliOptions = Whitebox.getInternalState(
				klass, "commandLineInterface");

		// all demo apps that use command line interface have a help option
		...
		for (int count = MAX_RANDOM_RETRIES; count > 0; count--) {
			// give me a sample command line in random order. the only
			// restriction is that the hadoop options have to be
			// first
			String[] commandLineArgs = getRandomizedCommandLine(
					(HashMap<String, String>) hadoopOptions.clone(),
					(HashMap<String, String>) demoAppCliArgs.clone());

			// check the command line against our options
			spiedCLI.checkCli(commandLineArgs);

			// iterate through all the options to test if they got parsed
			// correctly
			Iterator<Option> optionIterator = demoAppCliOptions.getOptions()
					.getOptions().iterator();

			// our counter to test the remaining args. all arguments - hadoop
			...
			while (optionIterator.hasNext()) {
				...
				if (next.hasArg()) {
					remaining += 2;
					testParsedOptions(next, demoAppCliArgs.get(next.getOpt()),
							spiedCLI);
				} else {
					remaining++;
			...
			// this test the remaining arguments after it was parsed by the
			// hadoop generic options parser
