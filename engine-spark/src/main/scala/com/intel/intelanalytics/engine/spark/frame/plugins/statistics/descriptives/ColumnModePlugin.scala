...
class ColumnModePlugin extends SparkCommandPlugin[ColumnMode, ColumnModeReturn] {
  ...
  override def doc: Option[CommandDoc] = Some(CommandDoc(oneLineSummary = "Calculate modes of a column.",
    extendedSummary = Some("""
                           ...
                           |                returned.
                           |                If the number of modes is > maxNumberOfModesReturned, then only
                           |                the first maxNumberOfModesReturned many modes (per a canonical
                           |                ordering) are returned.
                           |
                           |            weight_of_mode : Weight of a mode.
                           |                If there are no data elements of finite weight > 0, the weight
                           |                of the mode is 0.
