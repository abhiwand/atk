...
class SparkEngine(sparkContextManager: SparkContextManager,
  ...
  def loadSimple(load: Load, user: UserPrincipal, invocation: SparkInvocation): DataFrame = {
    ...
    if (load.source.isFrame) {
      ...
      val additionalData = frames.loadFrameRdd(ctx, frames.expectFrame(load.source.uri.toInt))
...
*/
  ...
  val assignSampleDoc = CommandDoc(oneLineSummary = "Assign classes to rows.",
    extendedSummary = Some("""
                            ...
                            |   The table receives an additional column that contains a random label generated by the
                            |   probability distribution function specified by a list of floating point values.
                            ...
                            |
                            |   Now the frame accessed by BigFrame *my_frame* has a new column named "sample_bin" and
                            |   each row contains one of the values "train", "test", or "validate".
...
*/
  ...
  def flattenColumnSimple(arguments: FlattenColumn, user: UserPrincipal, invocation: SparkInvocation): DataFrame = {
    ...
    val flattenedRDD = FlattenColumnSparkOps.flattenRddByColumnIndex(columnIndex, arguments.separator, rdd)
  ...
  */
  ...
  val columnModeDoc = CommandDoc(oneLineSummary = "Calculate modes of a column.",
    extendedSummary = Some("""
                            ...
                            |   All data elements of weight <= 0 are excluded from the calculation,
                            |   as are all data elements whose weight is NaN or infinite.
                            ...
                            |       Maximum number of modes returned. If this parameter is not provided, it defaults to 1.
                            |
                            |   Returns
                            |   -------
                            |   dictionary
                            ...
                            |               If the number of modes is > maxNumberOfModesReturned, then only the first maxNumberOfModesReturned many modes
...
TODO: delete me, code moved to separate plugin files
  ...
  val columnMedianDoc = CommandDoc(oneLineSummary = "Calculate (weighted) median of a column.",
    extendedSummary = Some("""
                            ...
                            |   The median is the least value X in the range of the distribution so that the cumulative
                            |   weight of values strictly below X is strictly less than half of the total weight and the
                            |   cumulative weight of values up to and including X is >= 1/2 the total weight.
                            |
                            |   All data elements of weight <= 0 are excluded from the calculation, as are all data elements
                            |   whose weight is NaN or infinite.
                            ...
                            |
                            |   weights_column : str (optional)
                            |       The column that provides weights (frequencies) for the median calculation.
                            |       Must contain numerical data. Uniform weights of 1 for all items will be used for
                            |       the calculation if this parameter is not provided.
                            |
                            |   Returns
                            |   -------
                            |   median
                            ...
                            |       If a weight column is provided and no weights are finite numbers > 0, None is returned.
                            |       Type of the median returned is that of the contents of the data column, so a column of
...
*/
  ...
  val columnSummaryStatisticsDoc = CommandDoc(oneLineSummary = "Calculate summary statistics of a column.",
    extendedSummary = Some("""
                            ...
                            |       Because this option affects the variance, it affects the standard deviation and the
                            |       confidence intervals as well.
                            |       This option is False by default, so that sample variance is the default form of
                            |       variance calculated.
                            |
                            |   Returns
                            |   -------
                            ...
                            |               None when there is a data element <= 0, 1.0 when there are no data elements.
                            |       *   variance:
                            |               None when there are <= 1 many data elements.
                            |               Sample variance is the weighted sum of the squared distance of each data element
                            |               from the weighted mean, divided by the total weight minus 1.
                            |               None when the sum of the weights is <= 1.
                            |               Population variance is the weighted sum of the squared distance of each data
                            |               element from the weighted mean, divided by the total weight.
                            |       *   standard_deviation:
                            |               The square root of the variance.
                            |               None when sample variance is being used and the sum of weights is <= 1.
                            |       *   valid_data_count:
                            ...
                            |               None when there are no elements of positive weight.
                            |       *   bad_row_count:
                            |               The number of rows containing a NaN or infinite value in either the data
                            |               or weights column.
                            |       *   good_row_count:
                            |               The number of rows not containing a NaN or infinite value in either the data
                            |               or weights column.
                            ...
                            |
                            |       where :math:`W` is sum of weights over valid elements of positive weight,
                            |       and :math:`M` is the weighted mean.
                            |
                            |   Population Variance
                            ...
                            |   --------------------
                            |   A row is bad when it contains a NaN or infinite value in either its data or weights column.
                            |   In this case, it contributes to bad_row_count; otherwise it contributes to good row count.
                            |
                            |   A good row can be skipped because the value in its weight column is <=0.
                            |   In this case, it contributes to non_positive_weight_count,
                            |   otherwise (when the weight is > 0) it contributes to valid_data_weight_pair_count.
                            |
                            |   Equations
                            |   ---------
 ...
 */
  ...
  */
  ...
  val dropColumnsDoc = CommandDoc(oneLineSummary = "Remove columns from the frame.",
    extendedSummary = Some("""
                            ...
                            |   Examples
                            |   --------
                            |   For this example, BigFrame object *my_frame* accesses a frame with columns *column_a*,
                            |   *column_b*, *column_c* and *column_d*.
  ...
  */
  ...
  def addColumnsSimple(arguments: FrameAddColumns, user: UserPrincipal, invocation: SparkInvocation) = {
    ...
    val ctx = invocation.sparkContext
    val frameId = arguments.frame.id
    val column_names = arguments.columnNames
    val column_types = arguments.columnTypes
  ...
  */
  ...
  val classificationMetricsDoc = CommandDoc(oneLineSummary = "Model descriptors",
    extendedSummary = Some("""
                            |   Based on the *metric_type* argument provided, computes the accuracy, precision, recall or
                            |   :math:`F_{\\beta}` measure for a classification model
                            |
                            |   *   When metric_type provided is 'f_measure': Computes the :math:`F_{\\beta}` measure for
                            |       a classification model.
                            |       A column containing the correct labels for each instance and a column containing the
                            |       predictions made by the model are specified.
                            |       The :math:`F_{\\beta}` measure of a binary classification model is the harmonic mean of
                            ...
                            |
                            |       For multi-class classification, the :math:`F_{\\beta}` measure is computed as the
                            |       weighted average of the :math:`F_{\\beta}` measure for each label, where the weight
                            |       is the number of instance with each label in the labeled column.
  ...
  val tallyPercentDoc = CommandDoc(oneLineSummary = "Computes a cumulative percent count.",
    extendedSummary = Some("""
                            |   A cumulative percent count is computed by sequentially stepping through the column values and
                            |   keeping track of the current percentage of the total number of times the specified
                            ...
                            |   -------
                            |   BigFrame
                            |       A new object accessing a new frame containing the original columns appended with
                            |       a column containing the cumulative percent counts
                            |
                            |   Examples
                            |   --------
                            ...
                            |
                            |   The BigFrame *cpc_frame* accesses a new frame that contains two columns, *obs* that
                            |   contains the original column values, and *obsCumulativePercentCount* that contains
                            |   the cumulative percent count::
                            |
                            |       cpc_frame.inspect()
                            |
  ...
  val tallyDoc = CommandDoc(oneLineSummary = "Computes a cumulative count.",
    extendedSummary = Some("""
                            |   A cumulative count is computed by sequentially stepping through the column values and
                            |   keeping track of the number of times the specified *count_value* has been seen,
                            ...
                            |   BigFrame
                            |       A new object accessing a new frame containing the original columns appended with
                            |       a column containing the cumulative counts
                            |
                            |   Examples
                            |   --------
  ...
  val cumPercentDoc = CommandDoc(oneLineSummary = "Cumulative percent sum.",
    extendedSummary = Some("""
                            |   Compute a cumulative percent sum.
                            |
                            |   A cumulative percent sum is computed by sequentially stepping through the column values
                            |   and keeping track of the current percentage of the total sum accounted for at the current value.
                            |
                            |   Parameters
                            |   ----------
                            |   sample_col : string
                            ...
                            |   BigFrame
                            |       A new object accessing a new frame containing the original columns appended with
                            |       a column containing the cumulative percent sums
                            |
                            |   Notes
                            |   -----
                            ...
                            |
                            |   The new frame accessed by BigFrame *cps_frame* contains two columns *obs* and
  ...
  val cumSumDoc = CommandDoc(oneLineSummary = "Computes a cumulative sum.",
    extendedSummary = Some("""
                            ...
                            |
                            |   A cumulative sum is computed by sequentially stepping through the column values and
                            |   keeping track of the current cumulative sum for each value.
                            |
                            |   Parameters
                            |   ----------
                            |   sample_col : string
                            ...
                            |   -------
                            |   BigFrame
                            |       A new object accessing a frame containing the original columns appended with
                            |       a column containing the cumulative sums
                            |
                            |   Notes
                            |   -----
                            |   This function applies only to columns containing numerical data.
                            ...
                            |
                            |   The BigFrame *cs_frame* accesses a new frame that contains two columns, *obs* that contains the
                            |   original column values, and *obsCumulativeSum* that contains the cumulative percent count::
                            |
                            |       cs_frame.inspect()
                            |
