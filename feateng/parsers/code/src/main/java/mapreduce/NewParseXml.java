...
public class NewParseXml {
    ...
    private static final Log LOG = LogFactory.getLog(NewParseXml.class);

    public static final String NAME = "NewParseXml";
	...
	@Override
		...
		for (KeyValue kv : columns.list()) {
		    ...
		    Deque<Element> tagList = new ArrayDeque<Element>();
		    for (int i = 0; i < children.size(); i++) {
	    		
			Element node = (Element) children.get(i);
			if (node.getText().trim().isEmpty()==false) {
			    
			    put.add(columnFamily, Bytes.toBytes(node.getName().trim()),Bytes.toBytes(node.getText().trim()));
		        }
			List nestedList = node.getChildren();
		        for (int j =0; j < nestedList.size(); j++) {
			    tagList.add( (Element) nestedList.get(j));
		        	
		        }
		       
		        int size = tagList.size();
			for (int j = 0; j < size; j++) {
			    Element tag = tagList.poll();
			    String nestedChild = node.getChildText(tag.getName());
			    put.add(columnFamily, Bytes.toBytes(node.getName().trim() +"_"+tag.getName()),Bytes.toBytes(nestedChild.trim()));
			}
		...
		System.err.println("Error: " + e.getMessage() + ", Row: " +
				   Bytes.toStringBinary(row.get()) + ", XML: " + value);
	...
	Job job = new Job(conf, "Parse data in " + input + ", write to " + output +
			  "(map only)");
	job.setJarByClass(NewParseXml.class);
