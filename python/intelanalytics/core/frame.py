...
class BigFrame(CommandSupport):
    ...
    def assign_sample(self, name):
        ...
        Parameters
        ----------
        sample_percentages : list of floating point values whose entries are non-negative and sum to 1. If the i'th
            entry of the  list is p, then then each row receives label i with independent probability p.

        sample_labels : Names to be used for the split classes. Optional. Defaults "TR", "TE", "VA" when there are three
            numbers given in split_percentages, defaults to Sample#0, Sample#1, ... otherwise.

        output_column : The name of the new column which holds the labels generated by the Optional. Defaults to "sample_bin".
        ...
        # Now my_frame has been appended with a new column named "sample_bin" and each row
