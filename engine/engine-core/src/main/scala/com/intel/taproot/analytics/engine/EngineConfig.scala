...
trait EngineConfig extends EventLogging {
  ...
  val sparkMaster: String = {
    val sparkMaster = config.getString("intel.taproot.analytics.engine.spark.master")
    if (sparkMaster == "") {
      "spark://" + hostname + ":7077"
    }
    else {
  ...
  val hiveLib: String = {
    config.getString("intel.taproot.analytics.engine.hive.lib")
  }

  val hiveConf: String = {
    config.getString("intel.taproot.analytics.engine.hive.conf")
  }
   ...
   */
  val reuseSparkContext: Boolean = config.getBoolean("intel.taproot.analytics.engine.spark.reuse-context")

  val defaultTimeout: FiniteDuration = config.getInt("intel.taproot.analytics.engine.default-timeout").seconds

  val fsRoot: String = config.getString("intel.taproot.analytics.engine.fs.root")

  val pageSize: Int = config.getInt("intel.taproot.analytics.engine.page-size")

  /* number of rows taken for sample test during frame loading */
  val frameLoadTestSampleSize: Int =
    config.getInt("intel.taproot.analytics.engine.command.frames.load.config.schema-validation-sample-rows")
  ...
  val frameLoadTestFailThresholdPercentage: Int =
    config.getInt("intel.taproot.analytics.engine.command.frames.load.config.schema-validation-fail-threshold-percentage")

  /**
   * A list of archives that will be searched for command plugins
   */
  val archives: List[String] = {
    config.getStringList("intel.taproot.analytics.engine.plugin.command.archives")
      .asScala
      .toList
  }
  ...
  def titanLoadConfiguration: SerializableBaseConfiguration = {
    createTitanConfiguration(config, "intel.taproot.analytics.engine.titan.load")
  }
   ...
   * @param titanPath Dot-separated expressions with Titan config, e.g., intel.taproot.analytics.engine.titan.load
  ...
  val sparkConfProperties: Map[String, String] = {
    ...
    val properties = config.getConfig("intel.taproot.analytics.engine.spark.conf.properties")
  ...
  val maxPartitions: Int = {
    config.getInt("intel.taproot.analytics.engine.auto-partitioner.max-partitions")
  }
   ...
   */
  val disableKryo: Boolean = config.getBoolean("intel.taproot.analytics.engine.spark.disable-kryo")
  ...
  val autoPartitionerConfig: List[FileSizeToPartitionSize] = {
    import scala.collection.JavaConverters._
    val key = "intel.taproot.analytics.engine.auto-partitioner.file-size-to-partition-size"
  ...
  val repartitionStrategy: SparkAutoPartitionStrategy.PartitionStrategy = {
    val strategyName = config.getString("intel.taproot.analytics.engine.auto-partitioner.repartition.strategy")
  ...
  val repartitionThreshold: Double = {
    val repartitionPercentage = config.getInt("intel.taproot.analytics.engine.auto-partitioner.repartition.threshold-percent")
  ...
  val frameCompressionRatio: Double = {
    val ratio = config.getDouble("intel.taproot.analytics.engine.auto-partitioner.repartition.frame-compression-ratio")
  ...
  val maxTasksPerCore: Int = {
    val tasksPerCore = config.getInt("intel.taproot.analytics.engine.auto-partitioner.default-tasks-per-core")
  ...
  val broadcastJoinThreshold = {
    val joinThreshold = config.getBytes("intel.taproot.analytics.engine.auto-partitioner.broadcast-join-threshold")
   ...
   */
  val sparkDriverMaxPermSize = config.getString("intel.taproot.analytics.engine.spark.conf.properties.spark.driver.maxPermSize")

  val sparkOnYarnNumExecutors = config.getInt("intel.taproot.analytics.engine.spark.conf.properties.spark.yarn.numExecutors")
   ...
   */
  val sparkAppJarsLocal: Boolean = config.getBoolean("intel.taproot.analytics.engine.spark.app-jars-local")
  ...
  val pythonWorkerExec: String = config.getString("intel.taproot.analytics.engine.spark.python-worker-exec")
  val pythonUdfDependenciesDirectory: String = config.getString("intel.taproot.analytics.engine.spark.python-udf-deps-directory")

  // val's are not lazy because failing early is better
  val metaStoreConnectionUrl: String = nonEmptyString("intel.taproot.analytics.metastore.connection.url")
  val metaStoreConnectionDriver: String = nonEmptyString("intel.taproot.analytics.metastore.connection.driver")
  val metaStoreConnectionUsername: String = config.getString("intel.taproot.analytics.metastore.connection.username")
  val metaStoreConnectionPassword: String = config.getString("intel.taproot.analytics.metastore.connection.password")
  val metaStorePoolMaxActive: Int = config.getInt("intel.taproot.analytics.metastore.pool.max-active")
  ...
  //gc variables
  val gcInterval = config.getDuration("intel.taproot.analytics.engine.gc.interval", TimeUnit.MILLISECONDS)
  val gcAgeToDeleteData = config.getDuration("intel.taproot.analytics.engine.gc.data-lifespan", TimeUnit.MILLISECONDS)

  val enableKerberos: Boolean = config.getBoolean("intel.taproot.analytics.engine.hadoop.kerberos.enabled")
  val kerberosPrincipalName: Option[String] = if (enableKerberos) Some(nonEmptyString("intel.taproot.analytics.engine.hadoop.kerberos.principal-name")) else None
  val kerberosKeyTabPath: Option[String] = if (enableKerberos) Some(nonEmptyString("intel.taproot.analytics.engine.hadoop.kerberos.keytab-file")) else None
