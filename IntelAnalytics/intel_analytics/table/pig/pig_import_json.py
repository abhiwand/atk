...
def main(argv):
    ...
    pig_statements.append("REGISTER %s" % (os.path.join(config['pig_lib'], 'elephant-bird-hadoop-compat-4.2.jar')))
    pig_statements.append("REGISTER %s" % (os.path.join(config['pig_lib'], 'json-simple-1.1.jar')))
    pig_statements.append("REGISTER %s" % (os.path.join(config['pig_lib'], 'elephant-bird-pig-4.2.jar')))
    pig_statements.append("REGISTER %s" % (os.path.join(config['pig_lib'], 'elephant-bird-core-4.2.jar')))
    ...
    pig_statements.append("STORE with_unique_row_keys INTO 'hbase://$OUTPUT' USING org.apache.pig.backend.hadoop.hbase.HBaseStorage('%s*');" % (config['hbase_column_family']));
    pig_script = "\n".join(pig_statements)
