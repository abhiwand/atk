...
object CfRequests {
  ...
  def getOrganizationsForUserId(token: String, userId: String): List[String] = {
    ...
    val uri = RestServerConfig.ccUri
  ...
  private def requestResourceIds(token: String, url: String): List[String] = {
    ...
    val response: HttpResponse = Await.result(pipeline(Get(url)), RestServerConfig.defaultTimeout)
  ...
  private def getUaaTokenPublicKey: String = {
    ...
    val uri = RestServerConfig.uaaUri
    ...
    Await.result(response, RestServerConfig.defaultTimeout).value
