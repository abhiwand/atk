...
public class ImportJsonFromFile {
	...
	@Override
		...
		type = conf.get("type").toString().trim();
		...
		//if(true) {
			...
			for (Integer i = 0; i < json.size(); i++) {
	            ...
	            if (val == null)
	            	...
	            	valString = val.toString();	
	            put.add(columnFamily, Bytes.toBytes(i.toString()),
	            					Bytes.toBytes(valString));
			}
	...
	if (cmd.hasOption("list")) type = "list";

	
	//System.out.println(type);
	conf.set("type", type);
	//System.out.println(conf.get("type"));
	// get details
