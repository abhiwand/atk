...
class MultiLineHeaderFooterInputFormatTest extends TestingSparkContextWordSpec with Matchers with BeforeAndAfter {
  ...
  def executeXmlInputFormat(file: File, startTags: List[String], endTags: List[String], isXml: Boolean = true): RDD[String] = {
    ...
    conf.setStrings(MultiLineHeaderFooterInputFormat.START_TAG_KEY, startTags: _*)
    conf.setStrings(MultiLineHeaderFooterInputFormat.END_TAG_KEY, endTags: _*)
    conf.setBoolean(MultiLineHeaderFooterInputFormat.IS_XML_KEY, isXml)
    val rows = sparkContext.newAPIHadoopFile[LongWritable, Text, MultiLineHeaderFooterInputFormat](file.getAbsolutePath,
      classOf[MultiLineHeaderFooterInputFormat], classOf[LongWritable], classOf[Text], conf)
