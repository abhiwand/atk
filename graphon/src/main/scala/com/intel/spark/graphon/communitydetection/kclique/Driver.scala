...
object Driver {
  ...
  def run(titanConfig: SerializableBaseConfiguration, sc: SparkContext, cliqueSize: Int, communityPropertyLabel: String) = {
    ...
    val kCliqueGraphGeneratorOutput = CliqueShadowGraphGenerator.run(enumeratedKCliques)

    // Run connected component analysis to get the cliques and corresponding communities
    val cliquesToComponents =
    ...
    val verticesToCommunitySets =
      VertexCommunityAssigner.run(cliquesToComponents)
    ...
    val gbVertexRDDBuilder: GBVertexRDDBuilder = new GBVertexRDDBuilder(gbVertices, verticesToCommunitySets)
