...
import com.intel.intelanalytics.engine.{ StageProgressInfo, TestingSparkContext }
import java.io.File
...
class SparkJobConcurrencyTest extends TestingSparkContext with Matchers {
  "Running multiple thread" should "keep isolation between threads when setting properties" in {
    ...
    val updater = new CommandProgressUpdater {
      override def updateProgress(commandId: Long, progress: List[Float], detailedProgress: List[StageProgressInfo]): Unit = {
        //do nothing
      }
    }
