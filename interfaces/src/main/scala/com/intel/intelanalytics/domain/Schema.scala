...
object DataTypes {
  ...
  case object int32 extends DataType {
    ...
    override def parse(s: String) = Try { s.toInt }
  }

  case object int64 extends DataType {
    override type ScalaType = Long

    override def parse(s: String): Try[int64.ScalaType] = Try { s.toLong }
  }

  case object float32 extends DataType {
    override type ScalaType = Float

    override def parse(s: String): Try[float32.ScalaType] = Try { s.toFloat }
  }
  ...
  case object float64 extends DataType {
    ...
    override def parse(s: String): Try[float64.ScalaType] = Try { s.toDouble }
  }

  case object string extends DataType {
    override type ScalaType = String

    override def parse(s: String): Try[string.ScalaType] = Try { s }
  }
