...
class SparkJoinITest extends TestingSparkContext with Matchers {
  ...
  "joinRDDs" should "join two RDD with inner join" in {
    ...
    val countryCode = sc.parallelize(id_country_codes).map(t => SparkOps.createKeyValuePairFromRow(t, List(0))).map(t => (t._1(0), t._2))
    val countryNames = sc.parallelize(id_country_names).map(t => SparkOps.createKeyValuePairFromRow(t, List(0))).map(t => (t._1(0), t._2))

    val result = SparkOps.joinRDDs(RDDJoinParam(countryCode, 2), RDDJoinParam(countryNames, 2), "inner")
    val sortable = result.map(t => SparkOps.createKeyValuePairFromRow(t, List(0))).map(t => (t._1(0), t._2)).asInstanceOf[RDD[(Int, Array[Any])]]
  ...
  "joinRDDs" should "join two RDD with left join" in {
    ...
    val countryCode = countryCodeRDD.map(t => SparkOps.createKeyValuePairFromRow(t, List(0))).map(t => (t._1(0), t._2))
    val countryNames = sc.parallelize(id_country_names).map(t => SparkOps.createKeyValuePairFromRow(t, List(0))).map(t => (t._1(0), t._2))
  ...
  "joinRDDs" should "join two RDD with right join" in {
    ...
    val sortable = result.map(t => SparkOps.createKeyValuePairFromRow(t, List(2))).map(t => (t._1(0), t._2)).asInstanceOf[RDD[(Int, Array[Any])]]
    val sorted = sortable.sortByKey(true)
