...
class AssignCommunitiesToVertexTest extends FlatSpec with Matchers with TestingSparkContext {
  ...
  "Assignment of communities to the vertices" should
    "produce the pair of original k-clique vertex and set of communities it belongs to" in new kCliqueVertexWithCommunityListTest {
      ...
      val vertexWithAssignedCommunities = AssignCommunitiesToVertex.run(rddOfRenamedIDWithCommunity, rddOfRenamedIDWithOriginalKClique)
