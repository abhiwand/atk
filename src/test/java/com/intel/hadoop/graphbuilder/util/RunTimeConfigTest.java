...
public class RunTimeConfigTest {
	...
	public final void tearDown() {
		...
		Whitebox.setInternalState(RuntimeConfig.class,
				"uniqueInstanceOfRuntimeConfig", mockedRuntimeConfig);
		runtimeConfig = null;
		spiedRuntime = null;
	}
	...
	public final void verify_unique_instance() {
		...
		RuntimeConfig runtimeConfig2 = RuntimeConfig
				.getInstance(RunTimeConfigTest.class);

		HashMap<String, String> config1 = Whitebox.getInternalState(
				runtimeConfig1, "config");
		HashMap<String, String> config2 = Whitebox.getInternalState(
				runtimeConfig2, "config");

		assertEquals("make sure the config of both runtimes is equal", config1,
				config2);
	}
	...
	public final void verify_unique_instance_class_specific_instantiation_first() {
		...
		Whitebox.setInternalState(spiedRuntime, "config",
				new HashMap<String, String>());
		RuntimeConfig runtimeConfig1 = RuntimeConfig
				.getInstance(RunTimeConfigTest.class);
	...
	public final void verify_grabbing_static_class_fields() {
		...
		RuntimeConfig staticTest = RuntimeConfig
				.getInstance(RunTimeConfigTest.class);

		HashMap<String, String> privateConfig = Whitebox.getInternalState(
				staticTest, "config");
		assertEquals("verify the config size", 3, privateConfig.size());
		assertEquals("verify the config value equals our static variable",
				testLoadingThroughReflection,
				privateConfig.get("testLoadingThroughReflection"));
		assertEquals("verify the config value equals our static variable",
				config.toString(), privateConfig.get("config"));
	}
	...
	public final void test_loading_grahbbuilder_config_namespace() {
		...
		assertTrue("verify value key one",
				spiedRuntime.getProperty("NAMESPACE_VALUEKEYONE").equals("one"));
	...
	public final void verify_getPropertyInt() {
		...
		assertTrue("verify value key one",
				spiedRuntime.getPropertyInt("NAMESPACE_VALUEKEYONE") == 1);
	}
	...
	public final void test_loading_config_key_parsing() {
		...
		assertTrue("verify value key two",
				spiedRuntime.getProperty("NAMESPACE_VALUEKEYTWO").equals("two"));
		assertTrue(
				"verify value key three",
				spiedRuntime.getProperty("NAME_SPACE_VALUE_KEY_THREE").equals(
						"three"));
	}
}
