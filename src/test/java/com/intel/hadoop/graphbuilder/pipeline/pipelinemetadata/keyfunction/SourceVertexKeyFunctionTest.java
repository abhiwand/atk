...
public class SourceVertexKeyFunctionTest {
	...
	public void testGetEdgeKey() throws Exception {
		...
		Edge<StringType> edge1 = new Edge<StringType>(sourceId, destId1,
				faulkner);
		Edge<StringType> edge2 = new Edge<StringType>(sourceId, destId2,
				new StringType("Simon"));

		assertEquals(keyFunction.getEdgeKey(edge1),
				keyFunction.getEdgeKey(edge2));

		StringType altSourceId = new StringType("The Curry");
		Edge<StringType> oddEdgeOut = new Edge<StringType>(altSourceId,
				destId1, faulkner);

		// technically, this could legally happen, if the underlying Java hash
		// function sent the edge IDs to the same
