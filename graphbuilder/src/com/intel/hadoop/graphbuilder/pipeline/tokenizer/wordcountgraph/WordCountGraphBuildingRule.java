 ...
 * rule (graph tokenizer) that converts wiki pages (presented as strings) into the 
 * property graph elements of the word count graph.
 * <p>
 * It is responsible for:
 * <ul>
 * <li> Generating the property graph schema that this graph construction rule generates 
 * and other meta-data required at set-up time.</li>
 * </ul>
 * </p>
 *
...
public class WordCountGraphBuildingRule implements GraphBuildingRule {
     ...
     * <p>There is one edge schema: It has the edge label "contains" and an integer 
	 * property, "wordCount".</p>
     */
    ...
    /**
     * Updates the Map Reduce job configuration with the state needed by 
	 * the {@code WordCountGraphTokenizer}.
     * @param {@code conf}  The job configuration that will store the state and 
	 * be passed to the {@code WordCountGraphTokenizer}.
     */
