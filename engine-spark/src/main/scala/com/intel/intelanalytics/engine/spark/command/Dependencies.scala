...
object Typeful {
  ...
  trait Searchable[T, Element] {
    def findAll(predicate: Element => Boolean) : List[Element]
  }

  trait LowPrioritySearchable {
    implicit def hlistishSearchable[T, L <: HList, Element](
                                                            implicit gen: Generic.Aux[Element, L],
                                                            s: Searchable[L, Element]
                                                             ) = new Searchable[T, Element] {
      def findAll(predicate: Element => Boolean)(element: Element) = s.findAll(predicate)(gen to element)
    }
  }
...
object Dependencies {
  ...
  type DependencyGraph = Graph[Computable, DiEdge[Computable]]

  def getDirectDependencies[A: Product](thing: A) = {
    everywhere()
  }

  def getDirectDependencies(arguments: JsObject): Seq[Computable] = {
    //TODO: Better to look up the argument type from the plugin and use reflection?
  }
