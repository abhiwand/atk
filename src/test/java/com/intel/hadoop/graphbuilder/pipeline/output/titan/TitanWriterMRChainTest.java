...
public class TitanWriterMRChainTest extends TestMapReduceDriverUtils {
	...
	public void test_hbase_to_vertices_to_titan_MR() throws Exception {
		...
		assertEquals(
				"check HTABLE_COLS_READ counter",
				gbVertexMapReduceDriver
						.getCounters()
						.findCounter(
								GBHTableConfiguration.Counters.HTABLE_COLS_READ)
						.getValue(), 5);
		assertEquals(
				"check HTABLE_ROWS_READ counter",
				gbVertexMapReduceDriver
						.getCounters()
						.findCounter(
		...
		assertEquals(
				...
				gbVertexMapReduceDriver
						...
						.getValue(), 1);
		assertEquals(
				"check NUM_VERTICES counter",
				gbVertexMapReduceDriver
						.getCounters()
						.findCounter(
								spiedVerticesIntoTitanReducer
										.getVertexCounter()).getValue(), 2);

		// set up our matching vertex to test against
		com.intel.hadoop.graphbuilder.graphelements.Vertex<StringType> vertex = new com.intel.hadoop.graphbuilder.graphelements.Vertex<StringType>(
				new StringType("Alice"));
		...
		com.intel.hadoop.graphbuilder.graphelements.Edge<StringType> edge = new com.intel.hadoop.graphbuilder.graphelements.Edge<StringType>(
				new StringType("Alice"), new StringType("GAO123"),
	...
	@Test
	public void test_hbase_vertices_to_titan_MR_two_hbase_results()
			throws Exception {
		...
		// set up our matching edge to test against
		com.intel.hadoop.graphbuilder.graphelements.Edge<StringType> edge = new com.intel.hadoop.graphbuilder.graphelements.Edge<StringType>(
				new StringType("Bob"), new StringType("INTELLABS"),
				new StringType("worksAt"));
		edge.setProperty(TitanGraphElementWriter.PROPERTY_KEY_SRC_TITAN_ID,
				new LongType(901L));
		...
		edge = new com.intel.hadoop.graphbuilder.graphelements.Edge<StringType>(
				new StringType("Alice"), new StringType("GAO123"),
				new StringType("worksAt"));
		edge.setProperty(TitanGraphElementWriter.PROPERTY_KEY_SRC_TITAN_ID,
				new LongType(902L));
