...
public class ArgumentBuilderTest {
	...
	public void test_setting_and_getting() {
		...
		assertTrue("verify exists with arg object",
				args.exists((Object) testArgumentValue));
		assertTrue("verify type matches",
				args.isType(testArgument, String.class));
		assertEquals("verify set value and the get value",
				args.get(testArgument), testArgumentValue);

		args = ArgumentBuilder.newArguments().with(testArgument,
				testArgumentValue);
	...
	public void test_get_with_default_value() {
		args = new ArgumentBuilder();
		assertTrue("make sure we get the default value when empty",
				(args.get(testArgument, Boolean.TRUE) == Boolean.TRUE));
		args.with(testArgument, testArgumentValue);
		assertTrue(
				"make sure we get the argument builder value when set and no the default value",
	...
	private void verifyExistsArgument() {
		...
		assertTrue("verify exists with arg object",
				args.exists((Object) testArgumentValue));
		assertTrue("verify type matches",
				args.isType(testArgument, testArgumentValue.getClass()));
		// verify against any other class to make sure it's not always returning
		// true
		assertFalse("verify type doesn't match",
				args.isType(testArgument, GraphElement.class));
	}

	private void verifyDoesntExistsArgument() {
		assertFalse("verify exists with arg name", args.exists(testArgument));
		assertFalse("verify exists with arg object",
				args.exists((Object) testArgumentValue));
	}
