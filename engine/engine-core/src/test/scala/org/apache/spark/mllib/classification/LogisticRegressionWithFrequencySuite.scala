...
class LogisticRegressionWithFrequencyClusterSuite extends TestingSparkContextFunSuite {

  test("task size should be small in both training and prediction using SGD optimizer") {
    val m = 4
    val n = 200000
    val points = sparkContext.parallelize(0 until m, 2).mapPartitionsWithIndex { (idx, iter) =>
