...
public class GradientDescentComputation extends BasicComputation<LongWritable, VertexDataWritable,
    ...
    public static final String MAX_SUPERSTEPS = "gd.maxSupersteps";
    /** Custom argument for learning rate */
    public static final String LEARNING_RATE = "gd.learningRate";
    /**
     * Custom argument for discount ratio on learning factor
     * learningRate(i+1) = discount * learningRate(i)
     * where discount should be in the range of (0, 1].
     * */
    public static final String DISCOUNT = "gd.discount";
    /** Custom argument for feature dimension */
    public static final String FEATURE_DIMENSION = "gd.featureDimension";
    /**
    ...
    public static final String MAX_VAL = "gd.maxVal";
    /** Custom argument for minimum edge weight value */
    public static final String MIN_VAL = "gd.minVal";
    /**
     * Custom argument for learning curve output interval (default: every iteration)
     * Since each GD iteration is composed by 2 super steps, one iteration
     * means two super steps.
     * */
    public static final String LEARNING_CURVE_OUTPUT_INTERVAL = "gd.learningCurveOutputInterval";
    ...
    public void preSuperstep() {
        ...
        learningRate = getConf().getFloat(LEARNING_RATE, 0.001f);
        discount = getConf().getFloat(DISCOUNT, 1.0f);
        if (discount < 0 || discount > 1) {
            throw new IllegalArgumentException("Discount factor should be in the range of (0, 1].");
        }
        ...
        // update learning rate at each iteration (every 2 super steps)
        if (getSuperstep() % 2 == 0) {
            learningRate *= Math.pow(discount, getSuperstep() / 2.0);
        }
    }
     ...
     */
    public static class GradientDescentMasterCompute extends DefaultMasterCompute {
        @Override
    ...
    /**
     * This is an aggregator writer for GD, which after each superstep will persist the
     * aggregator values to disk, by use of the Writable interface.
     */
    public static class GradientDescentAggregatorWriter extends DefaultImmutableClassesGiraphConfigurable
        ...
        private static void setFilename(long applicationAttempt) {
            FILENAME = "gd-learning-report_" + applicationAttempt;
        }
