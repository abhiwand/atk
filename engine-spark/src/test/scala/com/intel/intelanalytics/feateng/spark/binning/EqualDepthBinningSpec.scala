...
class EqualDepthBinningSpec extends Specification {

  "Equal depth binning" should {
    ...
    "create the correct number of bins" in new TestingSparkContext {
      ...
      // Compute equal depth bins
      val binnedData = EqualDepthBinning.bin(inputRdd, 5)
    ...
    "create equal depth bins" in new TestingSparkContext {
      ...
      // Validate
      val checkBins = binnedData.groupBy(binning ⇒ binning._2).map(grouping ⇒ grouping._2.size).toArray

      checkBins.distinct(0) mustEqual 2
    }
    ...
    "not split elements with same value across bins" in new TestingSparkContext {
      ...
      // Validate
      binnedData.map(binMapping ⇒ binMapping._2) mustEqual Array(1, 1, 1, 1, 1, 1, 1, 1, 2, 2)
    }
