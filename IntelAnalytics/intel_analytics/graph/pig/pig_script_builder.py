...
class GBPigScriptBuilder(object):
    ...
    def _build_hbase_table_load_statement(self, table_name, pig_alias):
        ...
        return "%s = LOAD 'hbase://%s' USING org.apache.pig.backend.hadoop.hbase.HBaseStorage('%s') AS (%s) ;" % (
        pig_alias, table_name, hbase_load_args, pig_schema)
            ...
            for i in range(field_count):
                try:
                    ...
                    if t != schema.feature_types[i]:
                        raise Exception("Encountered property %s with different types %s and %s" % (
                        schema.feature_names[i], schema.feature_types[i], t))
                except:
                    pass
    ...
    def _build_store_graph_statement(self, pig_alias, gb_conf_file, vertex_list, edge_list, type_table, other_args):
        ...
        return "STORE_GRAPH(%s, '%s', '%s', '%s', '%s');" % (
        pig_alias, gb_conf_file, type_info, edge_schemata_str, other_args)

    def _build_store_graph_infer_schema_statement(self, merged_elements, gb_conf_file, other_args):
        #See STORE_GRAPH_INFER_SCHEMA macro in graphbuilder.pig for the macro definition
        if other_args == None:
            other_args = ''
        return "STORE_GRAPH_INFER_SCHEMA(%s, '%s', '%s');" % (merged_elements, gb_conf_file, other_args)

    def _build_load_titan_statement(self, directed, gb_conf_file, source_table_name, vertex_list, edge_list,
                                    other_args):
        if other_args == None:
            other_args = ''
        ...
        return "LOAD_TITAN('%s', '%s', '%s', '%s', '%s');" % (
        source_table_name, vertex_rule, edge_rule, gb_conf_file, other_args)
    ...
    def create_pig_bulk_load_script(self, gb_conf_file, source_frame, vertex_list, edge_list,
        ...
        if registered_vertex_properties == None and registered_edge_properties == None:
            ...
            if need_dynamic_edge_label:
                ...
                statements.append(
                    "DEFINE CreatePropGraphElements com.intel.pig.udf.eval.CreatePropGraphElements('-v %s %s');" % (
                    vertex_rule, edge_rule))
                statements.append("pge = FOREACH base_graph GENERATE FLATTEN(CreatePropGraphElements(*));")
                statements.append("merged = MERGE_DUPLICATE_ELEMENTS(pge);")
                merged_elements = "merged"
                statements.append(self._build_store_graph_infer_schema_statement(merged_elements, gb_conf_file,
                                                                                 other_args))
            else:
                statements.append(
                    self._build_load_titan_statement(directed, gb_conf_file, source_table_name, vertex_list, edge_list,
                                                     other_args))
        else:
            edges = ' '.join(map(lambda e: '"' + self.edge_str(e) + '"', edge_list))
            vertex_rule = ' '.join(map(lambda v: '"' + self.vertex_str(v) + '"', vertex_list))
            ...
            if registered_vertex_properties:#we have some extra vertex properties to register
                statements.append(
                    self._build_hbase_table_load_statement(registered_vertex_properties.source_frame._table.table_name,
                                                           "graph_vp"))
                statements.append(
                    "DEFINE CreatePropGraphElements com.intel.pig.udf.eval.CreatePropGraphElements('-v %s');" % (
                    self._get_gb_vertex_rule(registered_vertex_properties)))
            ...
            if registered_edge_properties:#we have some extra edge properties to register
                statements.append(
                    self._build_hbase_table_load_statement(registered_edge_properties.source_frame._table.table_name,
                                                           "graph_ep"))
                edge = registered_edge_properties.edge
                edge_rule = self._get_gb_edge_rule(registered_edge_properties, directed)
                statements.append(
                    "DEFINE CreatePropGraphElements com.intel.pig.udf.eval.CreatePropGraphElements('-v %s %s %s');" % (
                    edge.source, edge.target, edge_rule))
