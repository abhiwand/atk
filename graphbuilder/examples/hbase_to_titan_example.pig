...
valid_employees = FILTER employees_with_valid_ids BY manager!='';

--GB requires the input data to be in HBase so
--we need to append HBase row keys to the input relation
final_relation = FOREACH valid_employees GENERATE FLATTEN(CreateRowKey(*));
...
LOAD_TITAN_NEW ( 'hbase://gb_input_table', 'cf:id cf:name cf:age cf:dept cf:manager cf:underManager',
               'id:chararray, name:chararray, age:chararray, dept:chararray, manager:chararray, underManager:chararray',
                ...
                '"name:String,age:String,dept:String,underManager:String"',
                 '"worksUnder,underManager"', '-O');