...
/**
 * Reads columns from an <code>HTable</code> and emits vertices and edges. Most of the
 * logic has been moved to a <code>BaseMapper</code> class that is also used by
 * <code>TextParsingMapper</code>.
 *
...
public class HBaseReaderMapper extends TableMapper<IntWritable, SerializedGraphElement> {
    ...
    /**
     * Most of the code has been removed from <code>setup</code> and moved to the
     * <code>BaseMapper</code> class. This makes everything less complex, including exception handling.
	 * Any errors during setup will be caught by the <code>BaseMapper</code> class and logged as fatal
	 * and a <code>system.exit</code> will be called.
     *
     ...
     * <code>contex.write</code> inside the base mapper class will be caught and logged as errors so we can
