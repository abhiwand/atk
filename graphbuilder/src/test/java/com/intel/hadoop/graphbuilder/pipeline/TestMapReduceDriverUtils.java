 ...
 * setup needed for output pipeline greatly reducing the setup needed to test
 * the hbase to vertices to titan mr pipeline and edges to titan reducer. All
 * the external decencies like titan, hbase are mocked out but otherwise this
 * will run the entire pipeline from command line parsing rules, tokenizer to
 * writting to titan. If it's needed to run the MR pipepline it's spied
 * otherwise it's mocked.
 * 
 * @see com.intel.hadoop.graphbuilder.pipeline.output.titan.TitanWriterMRChain
 * @see GBMapReduceDriver
 */
@RunWith(PowerMockRunner.class)
@PrepareForTest({ IntermediateEdgeWriterReducer.class,
		VerticesIntoTitanReducer.class, HBaseReaderMapper.class,
...
public abstract class TestMapReduceDriverUtils {
	...
	public void tearDown() {
		...
		/*
		 * propertyGraphElements = null; spiedVertexPropertyGraphElements =
		 * null; spiedEdgePropertyGraphElements = null;
		 */
	...
	/**
	 * create a spied instance of IntermediateEdgeWriterReducer to be used by
	 * the IntermediateEdgeWriterReducer driver
	 * 
	...
	protected IntermediateEdgeWriterReducer newEdgesIntoTitanReducer() {
		spiedIntermediateEdgeWriterReducer = (IntermediateEdgeWriterReducer) newSpy(
				spiedIntermediateEdgeWriterReducer,
				IntermediateEdgeWriterReducer.class);
		try {
			PowerMockito
					.doReturn(titanGraph)
					.when(spiedIntermediateEdgeWriterReducer,
							method(IntermediateEdgeWriterReducer.class,
									getTitanGraphInstance,
	...
	/**
	 * create a spied instance of VerticesIntoTitanReducer to be used by the
	 * VerticesIntoTitanReducer driver
	 * 
	...
	protected VerticesIntoTitanReducer newVerticesIntoTitanReducer() {
		spiedVerticesIntoTitanReducer = (VerticesIntoTitanReducer) newSpy(
				spiedVerticesIntoTitanReducer, VerticesIntoTitanReducer.class);
		try {
			PowerMockito
					.doReturn(titanGraph)
					.when(spiedVerticesIntoTitanReducer,
							method(VerticesIntoTitanReducer.class,
									getTitanGraphInstance,
									Reducer.Context.class))
					.withArguments(any(Reducer.Context.class));
		} catch (Exception e) {
	...
	/**
	 * create a spy of the HbaseReaderMapper class that will be used by the
	 * HBaseReaderMapper driver
	 * 
	...
	protected HBaseReaderMapper newHBaseReaderMapper() {
		spiedHBaseReaderMapper = (HBaseReaderMapper) newSpy(
				spiedHBaseReaderMapper, HBaseReaderMapper.class);
		return spiedHBaseReaderMapper;
	}

	/**
	 * create a new IntermediateEdgeWriterReducer driver and stub the
	 * context.getMapOutputValueClass method call. This driver gets used
	 * directly it's not used has part of a MapReduceDriver.
	 * 
	 * @see com.intel.hadoop.graphbuilder.pipeline.output.titan.IntermediateEdgeWriterReducer
	 * 
	...
	protected ReduceDriver newEdgeReducerDriver() {
		...
		edgesReduceDriver = newReduceDriver(spiedIntermediateEdgeWriterReducer,
				"edgeReducerContextMock");

		PowerMockito.when(edgeReducerContextMock.getMapOutputValueClass())
				.thenReturn(klass);
	...
	/**
	 * create a new VerticesIntoTitanReducer driver and stub the
	 * context.getMapOutputValueClass method call
	 * 
	 * @see VerticesIntoTitanReducer
	 * 
	...
	protected ReduceDriver newVerticesIntoTitanReducerDriver() {
		...
		verticesReduceDriver = newReduceDriver(spiedVerticesIntoTitanReducer,
				"vertexReducerContextMock");

		PowerMockito.when(vertexReducerContextMock.getMapOutputValueClass())
				.thenReturn(klass);
	...
	/**
	 * create our new hbase reader map driver and get the mocked context and
	 * stub the context.getMapOutputValueClass method call
	 * 
	 * @see HBaseReaderMapper
	 * 
	...
	protected MapDriver newHbaseReaderMapperDriver() {
		...
		PowerMockito.when(mapContext.getMapOutputValueClass())
				.thenReturn(klass);
	...
	/**
	 * Run prerequisites needed for the hbase->vertex map reduce driver and
	 * create our new MapReduceDriver
	 * 
	 * @see GBMapReduceDriver
	 * @see HBaseReaderMapper
	 * @see VerticesIntoTitanReducer
	...
	protected MapReduceDriver newVertexHbaseMR() {
		...
		gbVertexMapReduceDriver = new GBMapReduceDriver(mapDriver,
				verticesReduceDriver);

		return gbVertexMapReduceDriver;
	}

	/**
	 * abstract the setup and running of MapReduceDriver for any other Hbase->?
	 * jobs we might have
	 * 
	 * @see GBMapReduceDriver
	 * 
	 * @param mapReduceDriver
	 *            the GBMapReduceDriver
	 * @param pairs
	 *            row key, column hbase data
	 * @return any reducer context.write output
	 * @throws IOException
	 */
	protected List<Pair<IntWritable, SerializedGraphElement>> runMapReduceDriver(
			GBMapReduceDriver mapReduceDriver,
	...
	/**
	 * takes row key, hbase Result pairs to run hbase->vertices MR job. Will
	 * also take care of adding the hadoop configuration to the driver.
	 * 
	 * @see HBaseReaderMapper
	 * @see VerticesIntoTitanReducer
	 * 
	 * @param pairs
	 *            row key, hbase Result pairs to pass to the hbase->vertices MR
	 *            job
	...
	/**
	 * add the hadoop config and inputs to the edge reduce driver and return the
	 * context.write output.
	 * 
	 * @param pairs
	 *            a Key value pair data to be used to test the Reducer
	 * @return any context.write output
	 * @throws IOException
	 */
	protected List<Pair<IntWritable, SerializedGraphElement>> runEdgeR(
			Pair<IntWritable, SerializedGraphElement[]>[] pairs)
			throws IOException {

		edgesReduceDriver.withConfiguration(conf);

		for (Pair<IntWritable, SerializedGraphElement[]> kv : pairs) {
			edgesReduceDriver.withInput(kv.getFirst(),
					Arrays.asList(kv.getSecond()));
		}
	...
	/**
	 * mock and set our logger into HbaseReaderMapper to verify the logging of
	 * error messages.
	 * 
	...
	protected Logger newLoggerMock() {
		if (loggerMock == null) {
			...
			Whitebox.setInternalState(HBaseReaderMapper.class, "LOG",
					loggerMock);
		}
		return loggerMock;
	}
	...
	/**
	 * a real hbase configuration we use with the map/reduce pipeline with the
	 * minimum set to get it working
	 * 
	...
	/**
	 * The graph building rules that are usually defined on the command line
	 * minus the command line option and broken out into string arrays if you
	 * have more than one per option
	 */
	protected void newGraphBuildingRules() {
		...
		HBaseGraphBuildingRule.packVertexRulesIntoConfiguration(conf,
				vertexRules);
		...
		HBaseGraphBuildingRule.packDirectedEdgeRulesIntoConfiguration(conf,
				directedEdgeRules);
	}
	...
	protected Mapper.Context newMapperContext() {
		if (mapperContextMock == null) {
			...
			PowerMockito.when(mapperContextMock.getMapOutputValueClass())
					.thenReturn(valClass);
		}
	...
	protected BaseMapper newBaseMapper() throws Exception {
		if (spiedBaseMapper == null) {
			...
			PowerMockito.whenNew(BaseMapper.class).withAnyArguments()
					.thenReturn(spiedBaseMapper);
		}
	 ...
	 */
	protected TitanGraphElementWriter newTitanMergedGraphElementWrite()
			throws Exception {
		spiedTitanMergedGraphElementWrite = (TitanGraphElementWriter) newSpy(
				spiedTitanMergedGraphElementWrite,
				TitanGraphElementWriter.class);

		PowerMockito.whenNew(TitanGraphElementWriter.class).withAnyArguments()
				.thenReturn(spiedTitanMergedGraphElementWrite);

		return spiedTitanMergedGraphElementWrite;
	}

	/**
	 * create a spied PropertyGraphElements to stub out methods later in our
	 * test
	 * 
	 ...
	 */
	/*
	 * protected void newPropertyGraphElements() throws Exception { //step some
	 * mocks that get set when we create our spy newVerticesIntoTitanReducer();
	 * newTitanMergedGraphElementWrite();
	 * 
	 * if(spiedVertexPropertyGraphElements == null){
	 * 
	 * spiedVertexPropertyGraphElements = spy(new
	 * PropertyGraphElements(spiedTitanMergedGraphElementWrite, null, null,
	 * vertexReducerContextMock, titanGraph,
	 * (SerializedGraphElement)valClass.newInstance(),
	 * spiedVerticesIntoTitanReducer.getEdgeCounter(),
	 * spiedVerticesIntoTitanReducer.getVertexCounter()));
	 *//**
	 * this will make sure our spied instance get returned when it's
	 * instantiated in the VerticesIntoTitanReducer.initPropertyGraphElements
	 * 
	 * @see VerticesIntoTitanReducer
	 */
	/*
	 * PowerMockito.whenNew(PropertyGraphElements.class).withAnyArguments().
	 * thenReturn(spiedVertexPropertyGraphElements); } }
	 */

	/**
	 * create our titan graph mock and assign it to our class a titanGraph class
	 * field and return.
	 * 
	 * @see TitanGraph
	 * 
	 ...
	 * 
	 * @param reducer
	 *            the hadoop reducer we are going to create the driver for
	 * @param contextFieldName
	 *            this clases's context field name so we can assign the mocked
	 *            context
	 * @return a new reduce driver
	 */
	protected ReduceDriver newReduceDriver(
			org.apache.hadoop.mapreduce.Reducer reducer, String contextFieldName) {
		ReduceDriver newDriver = ReduceDriver.newReduceDriver(reducer);

		Field field;

		try {
			field = TestMapReduceDriverUtils.class
					.getDeclaredField(contextFieldName);
	 ...
	 * 
	 * @param inst
	 *            the object that will hold the mock
	 * @param klass
	 *            the class we are mocking
	 * @return new mock
	 */
	 ...
	 * 
	 * @param object
	 *            the object that will hold the spied intance
	 * @param klass
	 *            the class we are going to create a spy for
	...
	protected Object newSpy(Object object, Class klass) {
		if (object == null) {
			try {
				...
				// if we get any of the thrown exceptions fail the test because
	 ...
	 * 
	 * @param vertexId
	 *            string vertex id. will be used to create the StringType vertex
	 *            id
	 * @param properties
	 *            a hash map with all the properties
	 * @return new StringType vertex
	 */
	public static final Vertex<StringType> newVertex(String vertexId,
			HashMap<String, WritableComparable> properties) {
		com.intel.hadoop.graphbuilder.graphelements.Vertex vertex = new com.intel.hadoop.graphbuilder.graphelements.Vertex<StringType>(
				new StringType(vertexId));
		for (Map.Entry<String, WritableComparable> entry : properties
				.entrySet()) {
	 ...
	 *            id
	 * @param propertyMap
	 *            a new property map for the vertex properties
	 * @return new StringType vertex
	 */
	public static final Vertex<StringType> newVertex(String vertexId,
			PropertyMap propertyMap) {
		com.intel.hadoop.graphbuilder.graphelements.Vertex vertex = new com.intel.hadoop.graphbuilder.graphelements.Vertex<StringType>(
				new StringType(vertexId), propertyMap);
		return vertex;
	}
	 ...
	 * 
	 * @param src
	 *            string edge src. will be used to create the new StringType
	 * @param dst
	 *            string edge dst. will be used to create the new StringType
	 * @param label
	 *            string edge label. will be used to create the new StringType
	 * @param propertyMap
	 *            a new property map for the edge properties
	 * @return new StringType edge
	 */
	public static final Edge<StringType> newEdge(String src, String dst,
			String label, PropertyMap propertyMap) {
		com.intel.hadoop.graphbuilder.graphelements.Edge edge = new Edge<StringType>(
				new StringType(src), new StringType(dst),
				new StringType(label), propertyMap);
		;
		return edge;
	}
	 ...
	 *            string edge label. will be used to create the new StringType
	 * @param properties
	 *            hashmap with all the desired edge properties
	 * @return new StringType Edge
	 */
	public static final Edge<StringType> newEdge(String src, String dst,
			String label, HashMap<String, WritableComparable> properties) {
		com.intel.hadoop.graphbuilder.graphelements.Edge edge = new Edge<StringType>(
				new StringType(src), new StringType(dst), new StringType(label));
		for (Map.Entry<String, WritableComparable> entry : properties
				.entrySet()) {
	...
	public static final Result sampleDataAlice() {
		...
		list.add(newKeyValue("row1", "cf", "underManager", "5yrs",
				"1381447886400"));
	...
	public static final Result sampleDataBob() {
		...
		list.add(newKeyValue("row2", "cf", "underManager", "1yrs",
				"1381447886400"));
	...
	/**
	 * help debug the wierdness with Hbase.Client.Results. prints all the sample
	 * data to see if any values are coming back null
	 * 
	 * @throws UnsupportedEncodingException
	 */
	public final void printSampleData(Result result)
			throws UnsupportedEncodingException {
	...
	/**
	 * will print the value for the given Column faimily for the hbase Result
	 * set. Will convert all CF and Qualifier strings to bytes with
	 * HConstants.UTF8_ENCODING before trying to read the qualifier value.
	 * 
	 * @param result
	 *            hbase result
	 * @param cf
	 ...
	 * @throws UnsupportedEncodingException
	 */
	public final void printSampleRow(Result result, String cf, String qualifier)
			throws UnsupportedEncodingException {
		System.out.println((cf + ":" + qualifier + " value: " + Bytes
	...
	/**
	 * small wrapper method to help us create KeyValues. All the input values
	 * are strings that will be converted to bytes with String.getBytes()
	 * 
	 * @param row
	 *            the row key string
	 * @param cf
	 *            the column family string
	 * @param qualifier
	 *            the qualifier string
	 * @param value
	 *            the value string
	 * @param time
	 *            the timestamp for the row as a string
	 * @return
	 */
	public static final KeyValue newKeyValue(String row, String cf,
			String qualifier, String value, String time) {
		KeyValue k1;
		try {
			k1 = new KeyValue(row.getBytes(HConstants.UTF8_ENCODING),
					cf.getBytes(HConstants.UTF8_ENCODING),
					qualifier.getBytes(HConstants.UTF8_ENCODING),
					Long.valueOf(time), KeyValue.Type.Put,
	...
	/**
	 * verify the value of the context.write(key, value). we don't care about
	 * the int writable because it's a key and will change no matter what our
	 * input data is
	 * 
	 * @param pair
	 *            the writable pair from the mapper
	 * 
	 */
	public final void verifyPairSecond(
			Pair<IntWritable, SerializedGraphElement> pair,
			SerializedGraphElement graphElement) {

		// checking the graphElement type of pair and graphElement i'm verifying
		// against
		assertEquals("graph types must match", pair.getSecond().graphElement()
				.getType(), graphElement.graphElement().getType());
		...
		/*
		 * i'm not making any distinction between vertex and edge. it makes the
		 * code cleaner i just have to watch out for nulls. verify all the edge,
		 * vertex values match
		 */
		assertTrue(
				"match src",
				(jobGraphElement.getSrc() == null && givenGraphElement.getSrc() == null)
						|| jobGraphElement.getSrc().equals(
								givenGraphElement.getSrc()));
		assertTrue(
				"match dst",
				(jobGraphElement.getDst() == null && givenGraphElement.getDst() == null)
						|| jobGraphElement.getDst().equals(
								givenGraphElement.getDst()));
		assertTrue(
				"match label",
				(jobGraphElement.getLabel() == null && givenGraphElement
						.getLabel() == null)
						|| jobGraphElement.getLabel().equals(
								givenGraphElement.getLabel()));
		assertTrue(
				"match id",
				(jobGraphElement.getId() == null && givenGraphElement.getId() == null)
						|| jobGraphElement.getId().equals(
								givenGraphElement.getId()));

		// verify all the edge/vertex properties match
		for (Writable writable : jobGraphElement.getProperties()
				.getPropertyKeys()) {
			...
			assertTrue(String.format(
					...
					value.toString()), givenGraphElement.getProperty(key)
					.equals(value));
		}
	}
}
