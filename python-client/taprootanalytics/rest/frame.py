...
status = {status}""".format(type=frame_type, name=frame_name, graph_data=graph_data, row_count=row_count, schema=schema, status=status)
    ...
    def inspect(self, frame, n, offset, selected_columns, wrap=None):
        ...
        if wrap:
            from taprootanalytics.core.ui import FrameInspection
            return FrameInspection(schema, data, wrap=wrap)
