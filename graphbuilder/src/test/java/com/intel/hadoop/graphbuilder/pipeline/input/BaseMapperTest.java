 ...
 * me to mock static methods, private methods and variables.
 */
...
public class BaseMapperTest {
	...
	public final void verify_edge_erro_counter_increment() {
		...
		Edge<StringType> brokenEdge = new Edge<StringType>(new StringType(
				"Alice"), new StringType(null), new StringType("worksAt"));

		mapVal.init(brokenEdge);

		org.apache.hadoop.mapreduce.Counter EDGE = mock(org.apache.hadoop.mapreduce.Counter.class);

		// mock our context.getCounter() call so
		// context.getCounter().increment(1) don't throw null
		PowerMockito.when(
				mapperContextMock.getCounter(BaseMapper
						.getEdgeWriteErrorCounter())).thenReturn(EDGE);

		spiedBaseMapper.incrementErrorCounter(mapperContextMock, mapVal);

		// verify context.getCounter(Counter).increment(1) Edge error Counter
		// got called
	...
	public final void verify_vertex_error_counter_increment() {
		...
		PowerMockito.when(
				mapperContextMock.getCounter(BaseMapper
						.getVertexWriteErrorCounter())).thenReturn(VERTEX);

		spiedBaseMapper.incrementErrorCounter(mapperContextMock, mapVal);

		// verify context.getCounter(Counter).increment(1) vertex error Counter
		// got called
	...
	public final void verify_contextWrite_logs_error_and_incrementErrorCounter_on_IOException()
		...
		PowerMockito
				...
				.when(mapperContextMock)
				.write(any(IntWritable.class),
						any(SerializedGraphElement.class));

		// this will stop the real call to incrementErrorCounter from happening
		// but the mock will still log the incrementErrorCounter for later
		// verification
		PowerMockito
				.doNothing()
				.when(spiedBaseMapper)
				.incrementErrorCounter(any(Mapper.Context.class),
						any(SerializedGraphElement.class));

		// any dummy vertex to call context write
		Vertex<StringType> dummyVertex = new Vertex<StringType>(new StringType(
				"test"));
		mapVal.init(dummyVertex);

		spiedBaseMapper.contextWrite(mapperContextMock, new IntWritable(1),
				mapVal);

		verify(spiedBaseMapper).incrementErrorCounter(
				any(Mapper.Context.class), any(SerializedGraphElement.class));
