...
/**
 * MrUnit MapReduceDriver was extended and modified lightly to allow
 * instantiation with a reduceDriver and mapDriver instead if the Map and Reduce
 * classes. Using drivers allows us to get a context mock before we run the map
 * reduce pipeline.
 * 
 * @see MapReduceDriver
 * 
 ...
 *            map input value
 * @param <K2>
 *            map output key / reduce input key
 * @param <V2>
 *            map output value / reduce input value
 * @param <K3>
 *            reduce output key
 * @param <V3>
 *            reduce output value
...
public class GBMapReduceDriver<K1, V1, K2, V2, K3, V3> extends
	...
	GBMapReduceDriver(MapDriver<K1, V1, K2, V2> mapper,
			ReduceDriver<K2, V2, K3, V3> reducer) {
	...
	public static <K1, V1, K2, V2, K3, V3> MapReduceDriver<K1, V1, K2, V2, K3, V3> newMapReduceDriver(
			final MapDriver<K1, V1, K2, V2> mapper,
			final ReduceDriver<K2, V2, K3, V3> reducer) {
		return new GBMapReduceDriver<K1, V1, K2, V2, K3, V3>(mapper, reducer);
	}

	/**
	 * runs the mapreduce driver. this is the MapReduceDriver's run method
	 * modified to use given map drivers not create ones
	 * 
	...
	public List<Pair<K3, V3>> run() throws IOException {
		...
		try {
			...
			// this was creating a new map driver i changed it to use the one i
			// give it
			mapOutputs.addAll(myMapDriver.withCounters(getCounters())
					.withConfiguration(getConfiguration()).withAll(inputList)
					.withMapInputPath(getMapInputPath()).run());

			// Run the reduce phase.
			LOG.debug("Starting reduce phase with reducer: " + myReduceDriver);
			return new ReducePhaseRunner<K3, V3>().runReduce(
					shuffle(mapOutputs), myReduceDriver);
		} finally {
	...
	private class ReducePhaseRunner<OUTKEY, OUTVAL> {
		private List<Pair<OUTKEY, OUTVAL>> runReduce(
				...
				final ReduceDriver<K2, V2, OUTKEY, OUTVAL> reduceDriver1)
				throws IOException {
			...
			if (!inputs.isEmpty()) {
				if (LOG.isDebugEnabled()) {
					...
					for (Pair<K2, List<V2>> input : inputs) {
						...
						LOG.debug("Reducing input (" + input.getFirst() + ", "
								+ sb + ")");
				...
				/*
				 * final ReduceDriver<K2, V2, OUTKEY, OUTVAL> myReduceDriver =
				 * ReduceDriver
				 */
				reduceDriver1.withCounters(getCounters())
				...
				if (outputFormatClass != null) {
					reduceDriver1.withOutputFormat(outputFormatClass,
							inputFormatClass);
				}
