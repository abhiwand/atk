...
class BigFrame(CommandSupport):
    ...
    def split_data(self, name):
        ...
        Parameters
        ----------
        split_percentages : list of floating point values whose entries are non-negative and sum to 1. If the i'th
            entry of the  list is p, then then each row receives label i with independent probability p.

        split_labels : Names to be used for the split classes. Optional. Defaults "TR", "TE", "VA" when there are three
            numbers given in split_percentages, defaults to Sample Bin #0, Sample Bin #1, ... otherwise.

        output_column : The name of the new column which holds the labels generated by the Optional. Defaults to "split_class".
            column name OR list of column names to be removed from the frame

        random_seed : Random seed used to generate the labels. Optional. Defaults to 0.

        Examples
        --------
        >>>
        # For this example, my_frame is a BigFrame object.
        >>> my_frame.split_data([0.3, 0.3, 0.4], ["train", "test", "validate"])
        # Now my_frame has been appended with a new column named "sample bin" and each row
        contains one of the values "train", "test", or "validate" in column sample_bin. Values in the other columns
        are unaffected.
        """
        pass
