...
//
//  def createCommandExecutor(name: String): CommandExecutor = {
//    val engine = mock[SparkEngine]
//    val commandStorage = mock[SparkCommandStorage]
//    val cmd: Command = Command(1, name, None, None, List(), false, None, null, null, None)
//    when(commandStorage.create(any(classOf[CommandTemplate]))).thenAnswer(ct => Command(1, ct.name, ct.arguments, None, None, List(), false, None, null, null, NOne))
...
//    val executor = createCommandExecutor("dummy")
//
...
//    var contextCountDuringExecution = 0
//    var containsKey1DuringExecution = false
//    val executor = createCommandExecutor("foo")
//
