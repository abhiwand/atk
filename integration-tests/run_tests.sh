...
nosetests $DIR/smoketests --nologcapture --with-xunitmp --xunitmp-file=$OUTPUT1 --processes=10 --process-timeout=90 --with-isolation
SMOKE_TEST_SUCCESS=$?
...
if [[ $SMOKE_TEST_SUCCESS == 0 ]] ; then
    ...
    nosetests $DIR/tests --nologcapture --with-xunitmp --xunitmp-file=$OUTPUT2 --processes=10 --process-timeout=90 --with-isolation
    TEST_SUCCESS=$?
fi
