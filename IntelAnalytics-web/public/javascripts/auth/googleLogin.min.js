    ...
    googleAuth = {
        ...
        handleAuthResultLogin: function(authResult) {
            if (authResult && !authResult.error) {
                ...
                $(".googleLoginButton").click(function(){
                    googleAuth.handleAuthClickLogin();
                });
        ...
        loginCallback: function(authResult){
            if(googleAuth.isValidApiResponse(authResult)){
                ...
                if(googleAuth.accessGranted(authResult)){
                    showSignInError();
                }else{
                    messages.showAccessRequired();
                }
            }
        },
        ...
        registerCallback: function(authResult) {
            if(googleAuth.isValidApiResponse(authResult)){
                ...
                if(token != null && token != undefined && token.access_token && token.client_id){
                    authResult.access_token = token.access_token;
                    authResult.client_id = token.client_id;
                }
        ...
        accessGranted: function(resp){
            if(resp.code != undefined && resp.code == 401){
                return false;
            }else if(resp.email != undefined && resp.email != ""){
                return true;
            }
        },
        isValidApiResponse: function(resp){
            if((resp == undefined || resp == null) || !googleAuth.accessGranted(resp) ){
                return false;
            }else{
                return true;
            }
