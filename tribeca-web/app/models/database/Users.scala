...
case class User(uid: Option[Long], givenName:String, familyName:String, email:String, phone:String,
                company:String, companyEmail:String, picture:String, registered:Boolean, clusterId:Option[String])
...
class Users extends Table[User]("user_info") {
  ...
  def * = uid.? ~ givenName ~ familyName ~ email ~ phone ~ company ~ companyEmail ~ picture ~ registered ~ clusterId.? <> (User, User.unapply _)
}
